...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calculate = () => {
      ...
      let result: unknown = config[key];
      const cap = (s: string) => s.charAt(0).toUpperCase() + s.slice(1);
      const k = cap(key as string);
      const dynamicConfig = config as Record<string, unknown>;

      if (w <= 480 && dynamicConfig[`mobile${k}`]) {
        result = dynamicConfig[`mobile${k}`];
      } else if (w <= 768 && dynamicConfig[`tablet${k}`]) {
        result = dynamicConfig[`tablet${k}`];
      } else if (w <= 1024 && dynamicConfig[`desktop${k}`]) {
        result = dynamicConfig[`desktop${k}`];
      }
